  ...
  return (
    <div className="bg-[#F8F9FB] min-h-screen flex font-['Outfit',sans-serif]">
      ...
      <div className="ml-[256px] flex-1">
        <Header title="Maintenance" subtitle="Let's check your Garage today" />

        <div className="p-10 flex gap-10 max-w-[1600px] mx-auto">
          {/* Main Content */}
          <div className="flex-1 space-y-8">
            ...
            <div className="bg-white rounded-[32px] border border-[#EEEFF2] shadow-sm overflow-hidden">
              <div className="p-8 flex justify-between items-center bg-white border-b border-[#EEEFF2]">
                ...
                <div className="flex gap-4">
                  <button className="flex items-center gap-2 px-6 py-2 border border-[#EEEFF2] rounded-xl text-sm font-bold text-[#04091E] hover:bg-gray-50 transition-all">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="text-left bg-white border-b border-[#EEEFF2]">
                      <th className="py-5 px-8 text-[10px] font-black text-[#A3A6B4] uppercase tracking-widest">Vehicle</th>
                      <th className="py-5 px-8 text-[10px] font-black text-[#A3A6B4] uppercase tracking-widest">Service Type</th>
                      <th className="py-5 px-8 text-[10px] font-black text-[#A3A6B4] uppercase tracking-widest">Date</th>
                      <th className="py-5 px-8 text-[10px] font-black text-[#A3A6B4] uppercase tracking-widest">Status</th>
                      <th className="py-5 px-8 text-[10px] font-black text-[#A3A6B4] uppercase tracking-widest">Cost</th>
                      <th className="py-5 px-8 text-[10px] font-black text-[#A3A6B4] uppercase tracking-widest">Notes</th>
                      ...
                      <tr key={record.id} className="hover:bg-gray-50/50 transition-all">
                        <td className="py-6 px-8">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-bold text-[#04091E] text-sm leading-tight">{record.vehicle}</p>
                              <p className="text-[10px] text-[#A3A6B4] font-medium tracking-wide">VIN: ...{record.vin?.slice(-6)}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-6 px-8">
                          <span className="text-[10px] font-black text-[#D72322] uppercase tracking-widest border border-red-100 px-3 py-1 rounded-md bg-red-50/30">
                            {record.service}
                          </span>
                        </td>
                        <td className="py-6 px-8 text-sm font-medium text-[#747681]">{record.date}</td>
                        <td className="py-6 px-8 relative">
                          <div
                            onClick={() => setStatusMenuId(statusMenuId === record.id ? null : record.id)}
                            className="flex items-center gap-2 cursor-pointer group"
                          >
                            <div className={`w-2 h-2 rounded-full ${record.status === 'COMPLETED' ? 'bg-[#10B981]' : record.status === 'OVERDUE' ? 'bg-[#D72322]' : 'bg-[#F97316]'}`} />
                            <span className={`text-[10px] font-black uppercase tracking-widest group-hover:underline ${record.status === 'COMPLETED' ? 'text-[#10B981]' : record.status === 'OVERDUE' ? 'text-[#D72322]' : 'text-[#F97316]'}`}>
                              {record.status}
                            </span>
                          </div>
                          {statusMenuId === record.id && (
                            <div className="absolute left-8 top-12 bg-white rounded-xl shadow-2xl border border-[#EEEFF2] py-2 w-40 z-30 overflow-hidden animate-in fade-in slide-in-from-top-2">
                              <p className="px-4 py-1 text-[8px] font-black text-[#A3A6B4] uppercase tracking-widest border-b border-[#EEEFF2] mb-1">Set Status</p>
                              <button onClick={() => handleStatusUpdate(record.id, 'COMPLETED')} className="w-full text-left px-4 py-2 text-xs font-bold text-[#10B981] hover:bg-green-50">Completed</button>
                              <button onClick={() => handleStatusUpdate(record.id, 'OPEN')} className="w-full text-left px-4 py-2 text-xs font-bold text-[#F97316] hover:bg-orange-50">Open</button>
                              <button onClick={() => handleStatusUpdate(record.id, 'OVERDUE')} className="w-full text-left px-4 py-2 text-xs font-bold text-[#D72322] hover:bg-red-50">Overdue</button>
                            </div>
                          )}
                        </td>
                        <td className="py-6 px-8 font-black text-[#04091E] text-sm">${record.cost.toLocaleString()}</td>
                        <td className="py-6 px-8 text-xs text-[#747681] font-medium max-w-[200px] truncate">{record.notes}</td>
                        <td className="py-6 px-8 text-right relative">
                          <button
                            onClick={() => setActionMenuId(actionMenuId === record.id ? null : record.id)}
                            className="p-2 text-[#A3A6B4] hover:text-[#04091E] transition-all"
                          >
                            <HiDotsVertical />
                          </button>
                          {actionMenuId === record.id && (
                            <div className="absolute right-12 top-12 bg-white rounded-xl shadow-2xl border border-[#EEEFF2] py-2 w-32 z-20 overflow-hidden animate-in fade-in slide-in-from-top-2">
                              <button
                                onClick={() => { setSelectedRecord(record); setShowViewModal(true); setActionMenuId(null); }}
                                className="w-full text-left px-4 py-2 text-sm font-bold text-[#04091E] hover:bg-gray-50 flex items-center justify-between"
                              >
                                View
                              </button>
                              <button
          ...
          <div className="w-[340px] space-y-8">
            {/* AI Insights Card */}
            <div className="bg-white rounded-[32px] border-l-[6px] border-[#D72322] p-8 shadow-sm">
              ...
              <div className="bg-red-50/30 rounded-2xl p-6 border border-red-50 mb-6">
                <p className="text-[10px] font-black text-[#D72322] uppercase tracking-widest mb-1">Next Maintenance</p>
                <p className="font-bold text-[#04091E] text-lg">Service in 1,240 km</p>
              </div>
              <div className="grid grid-cols-2 gap-4 mb-8">
                <div>
                  <p className="text-[10px] font-black text-[#A3A6B4] uppercase tracking-widest mb-1">Est. Cost</p>
                  <p className="font-black text-[#04091E] text-lg">$540.00</p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-[#A3A6B4] uppercase tracking-widest mb-1">Confidence</p>
                  <p className="font-black text-[#04091E] text-lg">92%</p>
                </div>
              </div>
              <button className="w-full bg-[#D72322] text-white py-4 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-red-100 hover:scale-[1.02] transition-all">
                Optimize Schedule
              </button>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-black text-[#04091E] text-xs uppercase tracking-widest font-['Outfit']">Upcoming</h3>
                <span className="bg-[#D72322] text-white text-[10px] px-2 py-0.5 rounded font-black">30D Window</span>
              </div>
              <div className="space-y-4">
                {data?.upcoming.map((u: any) => (
                  <div key={u.id} className="bg-white p-5 rounded-2xl border border-[#EEEFF2] flex items-center gap-4 group cursor-pointer hover:border-[#D72322]/20 transition-all">
                    <div className="p-3 bg-[#F8F9FB] rounded-xl text-[#A3A6B4] group-hover:text-[#D72322] transition-colors">
                      <HiCalendar className="text-xl" />
                    </div>
                    <div className="flex-1">
                      <p className="font-bold text-[#04091E] text-sm leading-tight">{u.title}</p>
                      <p className="text-[10px] text-[#A3A6B4] font-medium mt-0.5">{u.date}</p>
                    </div>
                    <div className="text-[#EEEFF2] group-hover:text-[#D72322] transition-colors">{">"}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Support/Tips */}
            <div className="bg-[#FFF8F8] p-6 rounded-[32px] border border-red-50">
              <div className="flex items-center gap-2 mb-3">
                <div className="w-1.5 h-1.5 rounded-full bg-[#D72322]" />
                <p className="font-black text-[#D72322] text-[10px] uppercase tracking-widest">Maintenance Tip</p>
              </div>
              <p className="text-[#747681] text-xs font-medium leading-relaxed">
            ...
            <div className="bg-white p-8 rounded-[32px] border border-[#EEEFF2] space-y-4">
              <p className="font-black text-[#04091E] text-[10px] uppercase tracking-widest">Need Assistance?</p>
...
function MiniStat({ label, value, trend, highlight, check, sparkle, color }: any) {
  ...
  return (
    <div className="bg-white p-8 rounded-[32px] border border-[#EEEFF2] shadow-sm relative group hover:border-[#D72322]/10 transition-all">
      <div className="flex flex-col gap-1">
        <h3 className="text-[10px] font-black text-[#A3A6B4] uppercase tracking-widest leading-none">{label}</h3>
        <div className="flex items-center gap-4 mt-2">
          <span className="text-4xl font-black text-[#04091E]">{value?.toLocaleString() || '0'}</span>
          {trend && (
      ...
      <div className={`absolute top-8 right-8 ${colorClasses[color]}`}>
...
function ViewMaintenanceModal({ record, onClose }: any) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white w-[800px] rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
...
function AddMaintenanceModal({ onClose, vehicles }: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white w-[880px] rounded-[32px] overflow-hidden">
        <div className="p-10 flex justify-between items-center border-b border-[#EEEFF2]">
          <h2 className="text-2xl font-black text-[#D72322]">Add Maintenance Request</h2>
          <button onClick={onClose} className="p-2 text-[#A3A6B4] hover:bg-gray-50 rounded-full transition-all"><HiX className="text-2xl" /></button>
        </div>

        <form onSubmit={handleSubmit} className="p-10 grid grid-cols-2 gap-y-10 gap-x-12">
...
function EditMaintenanceModal({ record, onClose, onDelete }: any) {
  const [formData, setFormData] = useState({
    ...
    service_date: record.date === "Today" ? new Date().toISOString().split('T')[0] : new Date(record.date).toISOString().split('T')[0],
    mileage: 45200, // Example
    cost: record.cost,
